...
class AIAnalyzeServiceIntegrationTest {
	...
	void createAnalyze_shouldCreateAnalyzeAndUpdateStatus() {
		...
		Answer a2 = new Answer(retrospect.getId(), questions.get(1).getId(), leaderId, "5", AnswerStatus.DONE);
		Answer a3 = new Answer(retrospect.getId(), questions.get(2).getId(), leaderId, "아주 재밌는 경험이다.", AnswerStatus.DONE);

		Answer a4 = new Answer(retrospect.getId(), questions.get(0).getId(), memberId, "100", AnswerStatus.DONE);
		Answer a5 = new Answer(retrospect.getId(), questions.get(1).getId(), memberId, "5", AnswerStatus.DONE);
		Answer a6 = new Answer(retrospect.getId(), questions.get(2).getId(), leaderId, "즐거웠어요", AnswerStatus.DONE);
